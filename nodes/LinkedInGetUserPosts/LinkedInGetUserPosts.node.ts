...
export class LinkedInGetUserPosts implements INodeType {
	description: INodeTypeDescription = {
		name: 'linkedInGetUserPosts',
		displayName: 'Periodix LinkedIn Get User Posts',
		description: 'List recent posts by a LinkedIn person or company — for pre-outreach research and personalization',
		icon: 'file:icon.svg',
		group: ['transform'],
		version: [1],
		subtitle: '={{ "Get posts: " + $parameter["identifier"] }}',
		defaults: { name: 'LinkedIn Get User Posts' },
		...
		properties: [
			...
			},
			{
				...
				default: '',
				required: true,
				placeholder: 'e.g. ACoAAB... or 12345',
				description: 'Internal ID of the author whose posts to fetch. For a person it starts with ACo/ADo; for a company use the numeric company ID (enable "Is Company").',
			},
			{
				...
				type: 'boolean',
				default: false,
				description: 'Whether the author is a company rather than a person',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const isCompany = this.getNodeParameter('isCompany', i) as boolean;
				...
				do {
					...
					const body: IDataObject = { profileId, identifier, isCompany, limit: pageLimit };
