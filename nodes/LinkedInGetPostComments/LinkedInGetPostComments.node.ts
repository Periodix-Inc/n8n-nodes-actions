...
export class LinkedInGetPostComments implements INodeType {
	description: INodeTypeDescription = {
		name: 'linkedInGetPostComments',
		displayName: 'Periodix LinkedIn Get Post Comments',
		description: 'List the comments on a LinkedIn post, with author details — a source of engaged leads',
		icon: 'file:icon.svg',
		group: ['transform'],
		version: [1],
		subtitle: '={{ "Get post comments" }}',
		defaults: { name: 'LinkedIn Get Post Comments' },
				...
				do {
					...
					const response = (await this.helpers.httpRequestWithAuthentication.call(
						...
						'periodixActionsApi',
						{
							method: 'POST',
							baseURL: BASE_URL,
							url: '/actions/get-post-comments/v1',
							body,
							json: true,
						},
